...
def test_search_id_movie(auth_api):
    ...
    with allure.step("Проверить, что ответ содержит наименование фильма и соответвует указанному id."):
...
def test_movie_search(auth_api):
    ...
    with allure.step("Проверить, что наименование фильма соответствует искомому в запросе."):
    ...
    with allure.step("Проверить, что год выпуска фильма соответствует искомому в запросе."):
    ...
    with allure.step("Проверить, что жанр фильма соответствует искомому в запросе."):
    ...
    with allure.step("Проверить, что страна производства фильма соответствует искомому в запросе."):
    ...
    with allure.step("Проверить, что рейтинг фильма соответствует искомому в запросе."):
...
def test_search_by_years_and_rating(auth_api):
    ...
    """
    with allure.step("Отправить запрос GET, который содержит параметры: релиз и рейтинг."):
    ...
    with allure.step("Проверить, что тело ответа содержит необходимые запрашиваемые атрибуты."):
        if data["docs"]:
            movie = data["docs"][0]
            ...
            with allure.step("Проверить, что тело ответа содержит необходимое количество результатов"):
                assert data["limit"] == 1, f"Expected limit of 1, but got {data['limit']}"
                assert data["page"] == 1, f"Expected page 1, but got {data['page']}"

            with allure.step("Проверить, что тело ответа содержит запрашиваемые года релиза."):
            ...
            with allure.step("Проверить, что рейтинг фильма в пределах указанного."):
...
def test_search_by_genre_and_interval(auth_api):
    ...
    """
    with allure.step("Отправить запрос GET, который содержит параметры: жанр и года релиза."):
    ...
    with allure.step("Проверить, что тело ответа содержит необходимые запрашиваемые атрибуты."):
        if data["docs"]:
            ...
            with allure.step("Проверить, что жанр, переданный в запросе, присутствует в жанрах фильма"):
                genres = [genre["name"] for genre in movie["genres"]]
                assert config.genre in genres, f"Genre '{config.genre}' not found in movie genres: {genres}"

            with allure.step("Проверить, что год фильма в пределах указанного диапазона"):
                min_year, max_year = map(int, config.years.split("-"))
                movie_year = movie["year"]
                assert min_year <= movie_year <= max_year, f"Movie year {movie_year} is not in the range {min_year}-{max_year}"
